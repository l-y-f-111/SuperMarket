...
namespace SuperMarket.Db.User.Operation
{
    public class UserOperation : IUserOperation
    {
        ...
        {
            var sql =
                "INSERT INTO public.user (user_id,user_pwd_hash,user_vip_level,user_vip_level_expire_time) VALUES(:user_id,:user_pwd_hash,:user_vip_level,:user_vip_level_expire_time)";
        ...
        {
            var sql = "DELETE FROM public.user where user_id=:userID";
        ...
        {
            var sql = "SELECT user_pwd_hash FROM public.user where user_id=:userID and user_pwd_hash=:pwdHash";
        ...
        {
            var sql =
                "SELECT vip_discount FROM public.user,public.vip where user_id=:userID and user_vip_level=vip_level";
        ...
        {
            var sql = "UPDATE public.user SET user_pwd_hash=:newValue where user_id=:userId";
        ...
        {
            var sql = "UPDATE public.user SET user_vip_level=:newValue where user_id=:userId";
            var flag = Db.Query(sql, 1, new[] { ("newValue", (object)newValue), ("userId", (object)userId) });
            if (flag == 1) return true;
            else return false;
        }
        ...
        {
            var sql = "UPDATE public.user SET user_vip_level_expire_time=:newValue WHERE user_id=:userId";
            var flag = Db.Query(sql, 1, new[] { ("newValue", (object)newValue), ("userId", (object)userId) });
            if (flag == 1) return true;
            else return false;
        }
